 ...
 */
  ...
  for (size = 8; size <= 1048576; size *= 2)
      ...
      for (i = 0; i < size / 8; i++)
	{
      ...
      if (myproc == 0)
	{
	  MPI_Send (a, size/8, MPI_DOUBLE, other_node, 0, MPI_COMM_WORLD);
	  MPI_Recv (b, size/8, MPI_DOUBLE, other_node, 0, MPI_COMM_WORLD, &status);
	}
      else
	{
	  MPI_Recv (b, size/8, MPI_DOUBLE, other_node, 0, MPI_COMM_WORLD, &status);
	  ...
	  if (last != 0)
	    b[last] += 1.0;
	  MPI_Send (b, size/8, MPI_DOUBLE, other_node, 0, MPI_COMM_WORLD);
	}
      t2 = MPI_Wtime ();
      time = 1.e6 * (t2 - t1);
      MPI_Barrier (MPI_COMM_WORLD);
 ...
 */
  ...
  for (size = 8; size <= 1048576; size *= 2)
      ...
      for (i = 0; i < size / 8; i++)
	{
      ...
      last = size / 8 - 1;
      MPI_Irecv (b, size/8, MPI_DOUBLE, other_node, 0, MPI_COMM_WORLD, &request);
      MPI_Barrier (MPI_COMM_WORLD);
      t1 = MPI_Wtime ();
      if (myproc == 0)
	{
  ...
  for (size = 8; size <= 1048576; size *= 2)
      ...
      for (i = 0; i < size / 8; i++)
	{
      ...
      last = size / 8 - 1;
      MPI_Irecv (b, size/8, MPI_DOUBLE, other_node, 0, MPI_COMM_WORLD, &request_b);
      MPI_Irecv (a, size/8, MPI_DOUBLE, other_node, 0, MPI_COMM_WORLD, &request_a);
      MPI_Barrier (MPI_COMM_WORLD);
      t1 = MPI_Wtime ();
