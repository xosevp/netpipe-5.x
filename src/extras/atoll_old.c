...
int send_message(ArgStruct *p, void *buf, int nbytes, int tag)
{
   ...
   while (1) {
      ...
      err = Atoll_send(p->prot.id_self, p->prot.id_nbor, (uint64) tag, 
...
//      printf("fopen(%s,r) failed, time #%d\n", nborport, t);
   ...
   err = Atoll_connect(p->prot.id_self, p->prot.id_nbor);
...
void SendTime(ArgStruct *p, double *t)
{
   ...
   err = send_message(p, &ntime, sizeof(unsigned long), 2);
...
void RecvTime(ArgStruct *p, double *t)
{
   ...
   err = recv_message(p, &ntime, sizeof(unsigned long), 2);
...
void SendRepeat(ArgStruct *p, int nrepeat)
{
   ...
   err = send_message(p, &nlrpt, sizeof(unsigned long), 3);
...
void RecvRepeat(ArgStruct *p, int *nrepeat)
{
   ...
   err = recv_message(p, &nlrpt, sizeof(unsigned long), 3);
...
{
   err = Atoll_disconnect( p->prot.id_self, p->prot.id_nbor);
