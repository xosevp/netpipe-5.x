...
void RecvData(ArgStruct *p)
{
   ...
   while( p->r_ptr[p->bufflen-1] != (p->tr ? p->expected+1 : p->expected-1) ) {
...
{
   if((p->buff=(char *)gpshmalloc(bufflen+MAX(p->soffset,p->roffset)))==(char *)NULL)
   {
      fprintf(stderr,"couldn't allocate memory\n");
      exit(-1);
   }
   p->buff[bufflen-1]='b'+p->tr;
   if((p->buff1=(char *)gpshmalloc(bufflen+p->soffset))==(char *)NULL)
   {
      fprintf(stderr,"Couldn't allocate memory\n");
      exit(-1);
   }
